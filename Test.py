...
image_Scene = cv2.imread("./kitkat_scene.png")
image_targetList = []

image_PathList = []
for path in os.listdir('./assets'):
    image_PathList.append('./assets/' + path)
    # print(path)
...
sift_Decriptor_Target = []
sift_keypoint_Scene, sift_Decriptor_Scene = SIFT.detectAndCompute(image_Scene,None)
sift_Decriptor_Scene = np.float32(sift_Decriptor_Scene)
...
for image in image_targetList:
    ...
    sift_Decriptor_Target.append(descriptor)

#ORB
orb_keypoint_Target = []
orb_Decriptor_Target = []
orb_keypoint_Scene, orb_Decriptor_Scene = ORB.detectAndCompute(image_Scene,None)

for image in image_targetList:
    keypoint, descriptor = ORB.detectAndCompute(image,None)
    orb_keypoint_Target.append(keypoint)
    orb_Decriptor_Target.append(descriptor)

#AKAZE
akaze_keypoint_Target = []
akaze_Decriptor_Target = []
akaze_keypoint_Scene, akaze_Decriptor_Scene = AKAZE.detectAndCompute(image_Scene,None)
akaze_Decriptor_Scene = np.float32(akaze_Decriptor_Scene)

for image in image_targetList:
    keypoint, descriptor = AKAZE.detectAndCompute(image,None)
    descriptor = np.float32(descriptor)
    akaze_keypoint_Target.append(keypoint)
    akaze_Decriptor_Target.append(descriptor)
...
#SIFT
sift_match_Result = []
for descriptor in sift_Decriptor_Target:
    sift_match_Result.append(flann.knnMatch(descriptor, sift_Decriptor_Scene, 2))
...
#ORB
orb_match_Result = []
for descriptor in orb_Decriptor_Target:
    match_Result = bfmatcher.match(descriptor, orb_Decriptor_Scene)
...
#AKAZE
akaze_match_Result = []
for descriptor in akaze_Decriptor_Target:
    akaze_match_Result.append(flann.knnMatch(descriptor, akaze_Decriptor_Scene, 2))
